...
def number_of_didigts(number: int) -> int:
...
def is_possible_to_solve(result: int, numbers: list[int]) -> bool:
    ...
    if result - last_number > 0:
        if not is_possible_to_solve(result - last_number, rest_numbers):
            if result % last_number == 0:
                if is_possible_to_solve(result // last_number, rest_numbers):
                    return True
            
            number_of_didigts_last_number = number_of_didigts(last_number)
            tenth_power = 10 ** number_of_didigts_last_number
            if (result - last_number) % tenth_power== 0:
                return is_possible_to_solve((result - last_number) // tenth_power, rest_numbers)
...
def main() -> None:
    assert solution(Path("day_07/data_test.txt")) == 11387

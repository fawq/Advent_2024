...
def solution(file_path: Path, min_gain: int = 100, max_taxicab_distance: int = 20) -> int:
    matrix = Matrix()
    ...
    for position_cost in all_position_costs:
        ...
        for possible_position, possible_cost in all_position_costs_dict.items():
            ...
            if 1 <= taxicab_distance <= max_taxicab_distance:
...
def main() -> None:
    assert solution(Path("day_20/data_test.txt"), 74) == 7

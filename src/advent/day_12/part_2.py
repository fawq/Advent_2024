...
class Matrix():
    ...
    def get_neighbors_with_direction(self, position: Position) -> list[tuple[Position, Direction]]:
    ...
    def get_sides_direction(self, neighbors: list[tuple[Position, Direction]]) -> set[Direction]:
        sides: set[Direction] = set([direction for direction in Direction])
        for neighbor in neighbors:
            sides.remove(neighbor[1])
        return sides
    
    def get_price(self) -> int:
        score: int = 0
        explored_positions: set[Position] = set()
        processed_positions: set[Position] = set()
        ...
        for position in self.get_all_positions():
            if position not in explored_positions:
                ...
                while not bfs_queue.empty():
                    ...
                    possible_neighbor: list[tuple[Position, Direction]] = self.get_neighbors_with_direction(current_position)

                    area += 1
                    possible_sides_direction: set[Direction] = self.get_sides_direction(possible_neighbor)
                    sides += len(possible_sides_direction)

                    for neighbor, _ in possible_neighbor:
                        if neighbor not in explored_positions:
                            explored_positions.add(neighbor)
                            bfs_queue.put(neighbor)
                        elif neighbor in processed_positions:
                            for side in possible_sides_direction:
                                if side in self.get_sides_direction(self.get_neighbors_with_direction(neighbor)):
                                    sides -= 1
...
def main():
    assert solution("day_12/data_test.txt") == 1206

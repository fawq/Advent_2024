...
from advent.utils.position import Direction, Position
...
def get_char(direction: Direction) -> str:
    match direction:
        case Direction.UP:
            return '^'
        case Direction.DOWN:
...
def get_taxicab_paths(source: Position, destination: Position, type_of_keyboard: Keyboard) -> set[str]:
    ...
    if add_row != 0:
        ...
        direction_char = get_char(direction)
...
def solution(file_path: Path, depth: int) -> int:
    sequences: list[str] = []
...
def part1_solution(file_path: Path) -> int:
    return solution(file_path, 3)

def part2_solution(file_path: Path) -> int:
    return solution(file_path, 26)

def main() -> None:
    assert part1_solution(Path("src/advent/day_21/data_test.txt")) == 126384
    print(part1_solution(Path("src/advent/day_21/data.txt")))
    assert part2_solution(Path("src/advent/day_21/data_test.txt")) == 154115708116294
    print(part2_solution(Path("src/advent/day_21/data.txt")))
